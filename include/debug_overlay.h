...
static void debug_print_table_entry(ProfileSlot *slot, float posX, float *posY)
{
  ...
  rdpq_text_print(NULL, FONT_BUILTIN_DEBUG_MONO, posX-10, *posY, "*");
  rdpq_set_prim_color(RGBA32(0xFF, 0xFF, 0xFF, 0xFF));

  if(slot->calls != 0) {
  rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, posX, *posY, "%-10.10s %5lu %7lu#", slot->name, slot->calls, slot->timeUs);
  } else {
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, posX, *posY, "%-10.10s     - %7lu#", slot->name, slot->timeUs);
  }
  *posY += 10;
}
...
void debug_draw_perf_overlay(float measuredFps)
{
    ...
    rdpq_text_print(NULL, FONT_BUILTIN_DEBUG_MONO, TABLE_POS_X, posY, "Tasks      Calls     Time");
    posY += 12;
    ...
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, TABLE_POS_X, posY, "Total (busy)     %7ld#", (uint32_t)timeTotalBusy);
    rdpq_set_prim_color(RGBA32(0xFF, 0xFF, 0xFF, 0xFF));
    ...
    //rdpq_text_print(NULL, FONT_BUILTIN_DEBUG_MONO, TABLE_POS_X, posY, "Waiting for:");
    //posY += 12;
    ...
    // Table - Total Waits
    rdpq_set_prim_color((color_t){0x99, 0x99, 0xEE, 0xFF});
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, TABLE_POS_X, posY, "Total (waiting)  %7ld#", (uint32_t)timeTotalWait);
    rdpq_set_prim_color(RGBA32(0xFF, 0xFF, 0xFF, 0xFF));
    ...
    rdpq_set_prim_color(RGBA32(0xFF, 0xFF, 0xFF, 0xFF));
    //rdpq_text_print((NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]-30, posY+2, "Total-Time");
    rdpq_text_print(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]-30, posY + 4 + (BAR_HEIGHT+BAR_BORDER), "RSP");
    rdpq_text_print(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]-30, posY + 4 + (BAR_HEIGHT+BAR_BORDER)*2, "RDP");

    rdpq_set_prim_color(RGBA32(0xAA, 0xAA, 0xAA, 0xFF));
    float fpsMarkerY = posY + 8 + (BAR_HEIGHT+BAR_BORDER)*3;
    rdpq_text_print(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]-30, fpsMarkerY, "FPS Target:");

    // FPS marker at bottom of lines
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0] + posFps60 - 20, fpsMarkerY, "60");
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0] + posFps30 - 20, fpsMarkerY, "30");
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0] + posFps20 - 20, fpsMarkerY, "20");

    rdpq_set_prim_color((color_t){0x99, 0x99, 0xEE, 0xFF});
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0] + 120, posY, "FPS: %.2f", measuredFps);
    rdpq_set_prim_color((color_t){0x99, 0x99, 0x99, 0xFF});
    rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0] + 208, posY, "(f:%lld)", profile_data.frame_count);
    ...
    if((busyWidth+idleWidth) < 150) {
      rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]+184, posY + 3 + (BAR_HEIGHT+BAR_BORDER), "%7lu#", timeTotalBusy + timeTotalWait);
      rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]+184, posY + 3 + (BAR_HEIGHT+BAR_BORDER)*2, "%7llu#", rdpTimeBusy);
    } else {
      rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]+2, posY + 3 + (BAR_HEIGHT+BAR_BORDER), "%lu#", timeTotalBusy + timeTotalWait);
      rdpq_text_printf(NULL, FONT_BUILTIN_DEBUG_MONO, barPos[0]+2, posY + 3 + (BAR_HEIGHT+BAR_BORDER)*2, "%llu#", rdpTimeBusy);
    }
}
